...
fn main() -> Result<()> {
    ...
    fs::create_dir_all(&args.work_dir).map_err(|e| format!("Failed to create work dir: {}", e))?;
    let temp_dir = Temp::new_dir_in(&args.work_dir)
        .map_err(|e| format!("Failed to create temp dir: {}", e))?;
    info!("Using temp dir: {:?}", temp_dir.to_path_buf());
